...
# Generate the noisy parameter map figure
# julia --project=. scripts/plot_parammap_figure.jl results_single_gpu.jld2 results_mpi_1n4g.jld2 results_mpi_2n8g.jld2
...
# unified range for T₁ and T₂ (outlier voxels are white)
clim_err = 30.0
...
for ri in 1:nrows
    for ci in 1:ncols
        ...
        if img === nothing
            ...
            gca().set_xticks(Float64[]);
            gca().set_yticks(Float64[])
...
suptitle("Noisy Phantom Reconstruction: 1 GPU vs 4 GPUs vs 8 GPUs", fontsize=16, color="#FFB347", fontweight="bold", y=0.98)
